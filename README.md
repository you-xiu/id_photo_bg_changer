...
- 点击或拖放导入照片，导入后自动完成抠图、构图与基础亮度校正
- 一键智能生成，处理期间显示进度状态并锁定冲突操作
- OpenCV 轮廓精修、发丝边缘去白边与局部蒙版修复
- YuNet 人脸检测、自动构图、眼线校正和导出前合规检查
- 可开关构图辅助线，包含中心线、眼线参考区、头顶安全区和肩部参考线
- 构图辅助线默认关闭，按需开启且不会进入导出文件
- 常驻成片质量面板，构图、尺寸、背景和亮度变化后自动复核
- 蓝、红、白、浅灰、深蓝及自定义背景色
- 一寸、二寸、小一寸、小二寸规格，显示像素、毫米、DPI 和 JPEG 体积限制
- 缩放、水平位置、垂直位置、旋转与亮度调整
- 抠图预览、原图/成片对比、透明人物 PNG 导出
- 撤销/重做蒙版修改，支持保留、删除和柔化三种修复方式
- DPI 元数据、JPEG 体积控制、6 寸照片打印排版
- 文件夹批量处理、红白蓝三色批量导出
- 批处理支持实时进度、中途取消、同名文件保护和逐张质量报告
- 保存后自动复核尺寸、DPI、体积和透明通道，采用原子写盘避免半成品
- 记忆常用规格、背景、DPI、体积限制、导出目录和最近文件
- 画布滚轮缩放、中键或右键平移、双击适合窗口
- 记忆最近导出目录、快速打开文件位置、未保存成片退出保护
